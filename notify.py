...
push_config = {
    ...
    'TG_BOT_TOKEN_SP': '',                 # tg 机器人的 TG_BOT_TOKEN_SP，例：1407203283:AAG9rt-6RDaaX0HBLZQq0laNOh898iFYaRQ
    'TG_USER_ID_SP': '',                   # tg 机器人的 TG_USER_ID_SP，例：1434078534
...
def telegram_bot(title: str, content: str) -> None:
    ...
    """
    if not push_config.get("TG_BOT_TOKEN_SP") or not push_config.get("TG_USER_ID_SP"):
        print("tg 服务的 bot_token 或者 user_id 未设置!!\n取消推送")
        return
    print("tg 服务启动")

    if push_config.get("TG_API_HOST"):
        url = f"https://{push_config.get('TG_API_HOST')}/bot{push_config.get('TG_BOT_TOKEN_SP')}/sendMessage"
    else:
        url = (
            f"https://api.telegram.org/bot{push_config.get('TG_BOT_TOKEN_SP')}/sendMessage"
        )
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    payload = {
        "chat_id": str(push_config.get("TG_USER_ID_SP")),
        "text": f"{title}\n\n{content}",
...
if push_config.get("TG_BOT_TOKEN_SP") and push_config.get("TG_USER_ID_SP"):
